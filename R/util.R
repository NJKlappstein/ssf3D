...
##' Convert location coordinates to that of environemntal data
##' 
...
#' @return Returns data.frame with all columns, sf geometry dropped, and replaced with coord columns 
#' @examples
...
sf_to_df <- function(data, coords = c("X","Y")){
  ...
  if(all(geom_types == "POINT")){
    ...
    warning("All columns except for coordinates are dropped for st geometry types other than 'POINT'.")
  }
