##' Function to calculate density of a Kent distribution
##' 
...
dKent <- function(kappa, rho, omega, delta, log = FALSE) {
  # calulate log density
  logC <- Directional::kent.logcon(k = kappa, b = rho)
...
##' Function to plot density of a Kent distribution on a sphere
##' 
##' @param kappa angular concentration parameter
##' @param rho ovalness parameter
##' 
...
plot_dKent <- function(kappa, rho) {
  ...
  p %>% plotly::layout(scene = list(xaxis=axx,yaxis=axy,zaxis=axz, 
                                    aspectmode = "manual",
